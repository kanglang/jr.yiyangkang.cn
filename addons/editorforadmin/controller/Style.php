...
class Style extends Controller {
	function get_article_style(){
	    $p = input ( 'p', 1 ); // 默认显示第一页数据
	    $row = 15;
		$group_id = input('group_id',0);
		$groupList = db('article_style_group') -> select();
		if($groupList){
			if($group_id==0){
			...
			$data = db('article_style') -> where(array('group_id'=>$group_id))->order ( 'id desc' )->page ( $p, $row )->select();
		}
		/* 查询记录总数 */
		$count = db ( 'article_style' )->where ( array('group_id'=>$group_id) )->count ();
