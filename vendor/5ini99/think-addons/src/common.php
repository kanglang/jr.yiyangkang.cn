...
Hook::add('app_init', function () {
    ...
    // Hook('action_begin');//初始控制器开始标签位 和线上（注释）有冲突2017-05-01 22:23:07
    // 非正是返回
    // if (!$autoload) {
    //     return;
    // }
    // 当debug时不缓存配置
    $config = App::$debug ? [] : Cache::get('addons_ckey', []);

    if (empty($config)) {
        // 读取addons的配置
        $config = (array)Config::get('addons_ckey');
        // 读取插件目录及钩子列表
        ...
        Cache::set('addons_ckey', $config);
    }
    Config::set('addons_ckey', $config);
});
...
Hook::add('action_begin', function () {
    ...
    $addons = (array)Config::get('addons_ckey.hooks');
    if (empty($data)) {
        // 初始化钩子
