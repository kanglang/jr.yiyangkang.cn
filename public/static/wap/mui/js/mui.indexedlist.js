...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		caleLayout: function() {//计算滚动条
			var self = this;
			//console.log(jQuery(window).height());
			var withoutSearchHeight = (jQuery(window).height() - jQuery('.indexed-list-search').height()) + 'px';
			self.el.bar.style.height = withoutSearchHeight;
			self.el.inner.style.height = withoutSearchHeight;
			var barItemHeight = ((jQuery(window).height() - jQuery('.main18-top').height()-40) / self.el.barItems.length) + 'px';
		...
		scrollTo: function(group) {
			...
			if (!groupElement || (self.hiddenGroups && self.hiddenGroups.indexOf(groupElement) > -1)) {
				return;
			}//console.log(self.el.inner.scrollTop+'---'+groupElement.offsetTop);
			self.el.inner.scrollTop = groupElement.offsetTop-jQuery('.main18-mid2').height()-90;
		},
