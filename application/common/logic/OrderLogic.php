...
class OrderLogic
{
	...
	{
		$data = I('post.');
		if( isset($data['public_params'])  ){
			unset($data['public_params']);
		}
        ...
        $data['imgs'] = $img['result'] ?: ($data['imgs'] ?implode($data['imgs'],','): ''); //兼容小程序，多传imgs
		...
		if(!empty($data['id'])){
			...
			//var_dump($data);exit;
			unset($data['consignee'],$data['mobile']);
			$db = db('return_goods');
			$result = $db->insertGetId($data);
		}

		if($result){
			return ['status'=>1,'msg'=>'申请成功','result'=>$result];
		}
    ...
    {
        $return_imgs = '';
        if ( isset($_FILES['return_imgs']['tmp_name']) && $_FILES['return_imgs']['tmp_name'] ) {
	...
	public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$user_note='',$pay_name='')
	{
		...
		$cartList = $this->cartList;//pe($cartList);

		// 0插入订单 order
		$address = M('UserAddress')->where("address_id", $address_id)->find();
		$shipping = M('Plugin')->where("code", $shipping_code)->cache(true,ZPSHOP_CACHE_TIME)->find();
		...
		$data = array(
				...
				'user_money'       =>$car_price['balance']*$car_price['fans_code'],//'使用余额',
				'coupon_price'     =>$car_price['couponFee'],//'使用优惠券',
				'code'             =>$car_price['code'],//'使用优惠码',
				'integral'         =>($car_price['pointsFee'] * tpCache('shopping.point_rate')), //'使用积分',
				'integral_money'   =>$car_price['pointsFee']*$car_price['fans_code'],//'使用积分抵多少钱',
				'total_amount'     =>($car_price['goodsFee'] + $car_price['postFee'])*$car_price['fans_code'],// 订单总额
				'order_amount'     =>$car_price['payables']*$car_price['fans_code'],//'应付款金额',
				'add_time'         =>time(), // 下单时间
		...
		{
			$goods = M('goods')->where("goods_id", $val['goods_id'])->cache(true,ZPSHOP_CACHE_TIME)->find();
			...
			$data2['spec_key']           = isset($val['spec_key'])?$val['spec_key']:''; // 商品规格
			$data2['spec_key_name']      = isset($val['spec_key_name'])?$val['spec_key_name']:''; // 商品规格名称
		...
		if($data['user_money']>0)
			M('Users')->where("user_id", $user_id)->setDec('user_money',$data['user_money']); // 抵扣余额
		// 4 删除已提交订单商品
		...
		$data4['user_money'] = -$data['user_money'];
...
//        M('goods_activity')->where(array('act_id'=>$act_id))->setInc('act_count',$pre_sell_price['goods_num']);
	 ...
	 * lico
	 */
	public function returnDiffForSup($order_goods,$user,$order_id,$order_sn){

		 // pe($order_goods);

		$upUser = db('users')->field('level')->where('user_id',$user['first_leader'])->find();
		if( $upUser && $upUser['level'] > $user['level'] ){

			foreach ($order_goods as $key => $value) {

				//有规格
				if( isset($value['spec_key']) && $value['spec_key'] ){

					$upGoods = db('spec_goods_price')->field('level0_price,level1_price,level2_price,price')->where('key',$value['spec_key'])->find();

				}else{//无规格

					$upGoods = db('goods')->field('level0_price,level1_price,level2_price,shop_price')->where('goods_id',$value['goods_id'])->find();
				...
				if( $column && $upGoods ){
					//上级价格
					$up_price = $upGoods[$column];//p($up_price);
					//差价
					$diff_price = ($value['member_goods_price']-$up_price ) * $value['goods_num'];
					//pe($diff_price);
					
					$desic = $user['nickname'].'购买：'.$value['goods_name'].'<br/>'.'获得差价佣金：'.$diff_price.'<br/>订单号：'.$order_sn.'<br/>时间：'.date('Y-m-d H:i:s',time());
					accountLog($user['first_leader'],$diff_price,0,$desic,$diff_price,$order_id,$order_sn);
				}
	 ...
	 * 向上二级返分销
	 */
	public function returnSecondDist($car_price,$user,$order_id,$order_sn){

		if( $user['level'] == 4 ){

			//p($car_price);p($user);
			$dist_config = tpCache('distribut');//p($dist_config);

			$ups[] = $user['first_leader'];
			$ups[] = $user['second_leader'];

			$parents = db('users')->where('user_id','In',$ups)->column('user_id,level');//p($parents);
			if( $user['first_leader'] && isset($parents[$user['first_leader']]) && $parents[$user['first_leader']] == 4 ){
				$dist_value = $car_price['pointsFee'] * ($dist_config['first_rate']/100);//p($dist_value);

				$desic = $user['nickname'].'总代购物<br/>获得一级分销：'.$dist_value.'<br/>订单号：'.$order_sn.'<br/>时间：'.date('Y-m-d H:i:s',time());
				accountLog($user['first_leader'],$dist_value,0,$desic,$dist_value,$order_id,$order_sn);
			}

			if( $user['second_leader'] && isset($parents[$user['second_leader']]) && $parents[$user['second_leader']] == 4 ){
				$dist_value = $car_price['pointsFee'] * ($dist_config['second_rate']/100);//p($dist_value);

				$desic = $user['nickname'].'总代购物<br/>获得二级分销：'.$dist_value.'<br/>订单号：'.$order_sn.'<br/>时间：'.date('Y-m-d H:i:s',time());
				accountLog($user['second_leader'],$dist_value,0,$desic,$dist_value,$order_id,$order_sn);
			}
