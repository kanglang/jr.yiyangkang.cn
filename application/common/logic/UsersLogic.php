...
class UsersLogic extends Model
{
     ...
     * $first_leader 邀请人id
     */
    public function thirdLogin($data=[],$invite=0,$first_leader=0){

        if(!empty($invite)){
            $invite = get_user_info($invite,0);//根据手机号查找邀请人 0,2
        }else{
            $invite = array();
        }
        $data['first_leader'] = $first_leader;  //推荐人id
        // 如果找到他老爸还要找他爷爷他祖父等
        if(!empty($data['first_leader'])&&0){
        ...
        if(is_array($invite) && !empty($invite)){
            $data['first_leader'] = $invite['user_id'];
            $data['second_leader'] = $invite['first_leader'];
            $data['third_leader'] = $invite['second_leader'];
        }/*  else if(config('invite') ==1 && empty($invite)){
            return array('status'=>-1,'msg'=>'请填写正确的推荐人手机号');
        } */

        // 成为分销商条件
        $distribut_condition = config('condition');
        if($distribut_condition == 0)  // 直接成为分销商, 每个人都可以做分销
            $data['is_distribut']  = 1;
        //以上为分销业务处理 2018-07-16 10:12:50
        //
        $user_id = db('users')->insertGetId($data);//插入用户数据
        if($user_id){
            if ($first_leader){
                //更新code
                Db::name('users')->where('user_id',$user_id)->update(['code'=>Logic::getCode($user_id)]);
                $pid_level=Db::name('users')->where('user_id',$first_leader)->value('level');
                //钻卡/公司招募
                if ($pid_level==3||$pid_level==4){
                    $this->writeApplyLog($user_id,$data['nickname'],$first_leader);
                }
            }
            //
            $pay_points = config('reg_integral'); // 会员注册赠送积分
            if($pay_points > 0){
            ...
            $token_data = [
                'token'=>md5($user_info['user_id'].mt_rand(111111,999999)),
            ...
            $user_info['token'] = $token_data['token'];//
            $ret_id = db('device_token')->insertGetId($token_data);
        }
        if($user_id){
            return array('status'=>1,'msg'=>'登陆成功','result'=>$user_info);
        }else{
            return array('status'=>0,'msg'=>'登陆失败','result'=>$user_info);
        }
    }

    public function writeApplyLog($user_id, $nickname, $pid) {
        $data = [
            'user_id'     => $user_id,
            'level'       => 2,
            'create_time' => time(),
        ...
        Db::name('user_apply_log')->insert($data);
    }
    ...
    public function get_info($user_id)
    {
        ...
        $user['message_count'] = $messageLogic->getUserMessageCount($user_id);
    ...
    public function add_address($user_id,$address_id = 0,$data){
        ...
        //检查手机格式
        if($post['consignee'] == '')
            return array('status'=>0,'msg'=>'收货人不能为空','result'=>'');
        if(!$post['province'] || !$post['city'] || !$post['district'])
            return array('status'=>0,'msg'=>'所在地区不能为空','result'=>'');
        if(!$post['address'])
            return array('status'=>0,'msg'=>'地址不能为空','result'=>'');
        if(!check_mobile($post['mobile']))
            return array('status'=>0,'msg'=>'手机号码格式有误','result'=>'');
        //添加地址
        if($address_id == 0) {
            $post['user_id'] = $user_id;
            $c = db('User_address')->where('user_id', $user_id)->count();
            if($c >= 20){
            ...
            $address_id = $user_address->allowField(true)->insertGetId($post);
            //如果设为默认地址
            $insert_id = db('user_address')->getLastInsID();
            $map['user_id'] = $user_id;
            $map['address_id'] = ['neq',$insert_id];

            if(isset($post['is_default']) && $post['is_default'] == 1)
                $user_address->save(['is_default'=>0],$map);
            if($address_id){
        ...
        //编辑地址
        if($address_id > 0){
            $address = db('user_address')->where(['address_id'=>$address_id,'user_id'=> $user_id])->find();
            if(isset($post['is_default']) && $post['is_default'] == 1 && $address['is_default'] != 1){
                db('user_address')->where('user_id',$user_id)->update(['is_default'=>0]);
            }
            if(isset($post['id']))unset($post['id']);
            $user_address = new UserAddress();
            $row = $user_address->allowField(true)->save($post,['address_id'=>$address_id,'user_id'=> $user_id]);
            if($row !== false){
                return array('status'=>1,'msg'=>'编辑成功','result'=>$address_id);
            }else{
                return array('status'=>0,'msg'=>'编辑失败','result'=>$address_id);
            }
        }
        return array('status'=>0,'msg'=>'数据错误','result'=>$address_id);
    }
    ...
    public function paypwd($user_id,$new_password,$confirm_password){
        ...
        return array('status'=>1,'msg'=>'修改成功','result'=>$url);
    }
    /**
    ...
    public function check_validate_code($code, $sender, $type ='email', $session_id=0 ,$scene = -1){
        ...
        //邮件证码是否开启
        $reg_smtp_enable = 0;//tpCache('smtp.regis_smtp_enable'); //2018-07-31 13:37:45

        if($type == 'email' && $reg_smtp_enable){
            if(!$reg_smtp_enable){//发生邮件功能关闭
                ...
                db('sms_log')->where(array('mobile'=>$sender , 'status'=>1))->update(['is_check'=>1]);
     ...
     * @return mixed
     */
    public function get_zpdistribution_info($user_id,$u_id,$type = 1,$account = 0){
        //获取个人粉丝信息
        if($user_id && $u_id){
            $son['subordinate'] = db('users')->where(['user_id'=>$u_id])->find();
            $son['reg_time'] = '注册时间' . ' ' . ($son['subordinate']['reg_time'] != 0 ? date('Y.m.d', $son['subordinate']['reg_time']) : '0000.00.00');
            $son['first'] = db('users')->where(['first_leader'=>$u_id])->count('first_leader');//子用户第一层
            $son['second'] = db('users')->where(['second_leader'=>$u_id])->count('second_leader');//子用户第二层
            $son['third'] = db('users')->where(['third_leader'=>$u_id])->count('third_leader');//子用户第三层
            $son['count'] = $son['first']+$son['second']+$son['third'];//子用户第三层

            $layer = db('users')->where(['user_id'=>$u_id])->field('first_leader,second_leader,third_leader')->find();
            ...
            return ['status'=>1,'msg'=>'获取数据成功','result'=>$son];
        }
        //获取全部粉丝信息
        if($user_id && !$u_id){
            $where_a =[
            ...
            if($type == 1){
                $count_leader = db('users')->whereOR($where_a)->select();
            }elseif($type == 2){
                $count_leader = db('users')->where($where_b)->select();
            }elseif($type == 3){
                $count_leader = db('users')->where($where_c)->select();
            }elseif($type == 6){
                if(!preg_match("/[^\d-., ]/",$account)){
                    $count_leader =  db('users')->where(function ($query) use ($account){
                ...
                $count_leader = db('users')->where($where_d)->select();
            }
            // $_count_leader = [];
            // //查询粉丝是否已购
            // foreach ($count_leader as $key => $val){
            //     $_c = $val;
            //     $memberorder = db('order')->where(['user_id'=>$val['user_id']])
            //         ->where('order_status','in',['2','4'])
            //         ->where('shipping_status&pay_status',1)
            //         ->where('')->field(['order_id'])->find();
            //     if($memberorder){
            //         $_c['memberorder'] = 1;
            //     } else {
            //         $_c['memberorder'] = 0;
            ...
            //     $_count_leader[] = $_c;
            // }
            return ['status'=>1,'msg'=>'获取数据成功','result'=>$count_leader];
        }
        return ['status'=>0,'msg'=>'缺少必要参数','result'=>''];
    }

    public function getOneLine($user_id){
        $count_leader = db('users')->where('first_leader',$user_id)->select();
        if(!empty($count_leader)){
            foreach($count_leader as $key =>$value){
                $count_leader[$key]['underling_number'] = $this->getOneLinesCount([$value['user_id']]);
            }
        }
        return ['status'=>1,'data'=>$count_leader];
    }

     function getOneLinesCount($user_list,$count = 0){
        if(count($user_list) > 0){
            $user_list = Db::name('users')->whereIn('first_leader',$user_list)->column('user_id');
            $count_1 = count($user_list);
            //列表
            $count += $count_1;
           return $this->getOneLinesCount($user_list,$count);
        }else{
            return $count;
        }
