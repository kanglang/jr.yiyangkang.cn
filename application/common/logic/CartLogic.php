...
class CartLogic extends Model
{
    ...
    public function setGoodsModel($goods_id)
    {
        ...
        $member_price = price_switch($this->user['level'],$this->goods);
        $this->goods['member_price'] = $member_price;
    }
    /**
     * 包含一个商品规格模型
     * @param $item_id
    ...
    public function setSpecGoodsPriceModel($item_id)
    {
        ...
        $member_price = price_switch($this->user['level'],[],$this->specGoodsPrice);
        $this->specGoodsPrice['member_price'] = $member_price;
    ...
    public function buyNow(){
        ...
        $buyGoods = [
            ...
            'member_goods_price'=>$this->goods['member_price'],
        ...
        if(empty($this->specGoodsPrice)){
            ...
            $buyGoods['member_goods_price'] = $this->specGoodsPrice['member_price'];
    ...
    private function addNormalCart(){
        if(empty($this->specGoodsPrice)){
            $price =  $this->goods['member_price'];
            $store_count =  $this->goods['store_count'];
        }else{
            //如果有规格价格，就使用规格价格，否则使用本店价。
            $price = $this->specGoodsPrice['member_price'];
            $store_count = $this->specGoodsPrice['store_count'];
        }
        // 查询购物车是否已经存在这商品
        if (!$this->user_id) {
        ...
        if ($userCartGoods) {
            ...
            if (!empty($this->goods['price_ladder'])) {
                ...
                $price = $goodsLogic->getGoodsPriceByLadder($userWantGoodsNum, $this->goods['member_price'], $price_ladder);
            }
            ...
            if (!empty($this->goods['price_ladder'])) {
                $goodsLogic = new GoodsLogic();
                $price_ladder = unserialize($this->goods['price_ladder']);
                $price = $goodsLogic->getGoodsPriceByLadder($this->goodsBuyNum, $this->goods['member_price'], $price_ladder);
            }
            $cartAddData = array(
...
//        $cartCheckAfterList = $cartList;
    ...
    public function AsyncUpdateCart($cart = [])
    {
        ...
        if ($cartList) {
            ...
            $cartPriceInfo = $this->getCartPriceInfo();
            $cartPriceInfo['cartList'] = $cartList;
            return ['status' => 0, 'msg' => '购物车没选中商品', 'result' => $cartPriceInfo];
        }
    }
