...
class MessageLogic extends Model
{
    ...
    public function getUserMessageCount( $user_id){
        ...
        $user_system_message_no_read = DB::name('user_message')
            ->alias('um')
            ->join('__SEND_MESSAGE__ m','um.message_id = m.message_id','LEFT')
     ...
     * @return array
     */
    public function getUserAllMessage($user_info){
        $this->checkPublicMessage($user_info);
        $user_message_no_read_where = array(
            'user_id' => $user_info['user_id'],
            'status' => 0,
        );
        $user_message_no_read = Db::name('user_message')
            ->alias('um')
            ->field('um.rec_id,um.user_id,um.category,um.message_id,um.status,m.send_time,m.type,m.message')
            ->join('__SEND_MESSAGE__ m','um.message_id = m.message_id','LEFT')
            ->where($user_message_no_read_where)
            ->select();
        if($user_message_no_read !== false){
            return ['status' => 1, 'msg' => '获取成功', 'result' => $user_message_no_read];
        }else{
    ...
    public function getUserMessageNotice($user_info){
        ...
        $user_system_message_no_read = Db::name('user_message')
            ...
            ->where($user_system_message_no_read_where)
            ->select();
        if($user_system_message_no_read !== false){
            return ['status' => 1, 'msg' => '获取成功', 'result' => $user_system_message_no_read];
        }else{
            return ['status' => 0, 'msg' => '获取失败', 'result' => ''];
        }
    }
     ...
     * @return array
     */
    public function getUserSellerMessage($user_info){
        $user_seller_message_no_read_where = array(
            'user_id' => $user_info['user_id'],
            'status' => 0,
            'm.category' => '1'
        );
        $user_seller_message_no_read = Db::name('user_message')
            ->alias('um')
            ->field('um.rec_id,um.user_id,um.category,um.message_id,um.status,m.send_time,m.type,m.message')
            ->join('__SEND_MESSAGE__ m','um.message_id = m.message_id','LEFT')
            ->where($user_seller_message_no_read_where)
            ->select();
        if($user_seller_message_no_read !== false){
            return ['status' => 1, 'msg' => '获取成功', 'result' => $user_seller_message_no_read];
        }else{
            return ['status' => 0, 'msg' => '获取失败', 'result' => ''];
        }
    }
    ...
    public function checkPublicMessage($user_info){
        ...
        $user_system_public_no_read = Db::name('send_message')->field('message_id')->where($message_where)->select();
        foreach ($user_system_public_no_read as $key) {
            db('user_message')->insert(['user_id'=>$user_info['user_id'],'message_id'=>$key['message_id'],'category'=>0,'status'=>0]);
        }
    }
     ...
     * @param $user_id
     * @param $message_id
     * @param int|string $category //0:系统消息|1：活动消息
     * @return array
     * @throws \think\Exception
     */
    public function setMessageForRead($user_id,$message_id,$category = ''){
        if(!empty($category) || $category == '0'){
            $del_where['category'] = $category;
        }
        if (!empty($user_id) && !empty($message_id)) {
            $set_where['user_id'] = $user_id;
            $set_where['message_id'] = $message_id;

            $updat_meg_res = db('user_message')->where($set_where)->update(['status'=>1]);
            if ($updat_meg_res !== false){
                return ['status'=>1,'msg'=>'设置成功','result'=>''];
            }
        }
        return ['status'=>0,'msg'=>'设置失败','result'=>''];
    }
     ...
     * @return array
     */
    public function setMessageRead($user_id,$category = ''){
        if(!empty($category) || $category == '0'){
            $set_where['category'] = $category;
        }
        if (!empty($user_id)) {
            $set_where['user_id'] = $user_id;
            $all_read = db('user_message')->where($set_where)->update(['status' => 1]);
            if($all_read !== false){
                return ['status' => 1, 'msg' => '设置成功', 'result' => ''];
            }
        }
     ...
     * @param $user_id
     * @param $message_id
     * @param string $category //0:系统消息|1：活动消息
     * @return array
     * @throws \think\Exception
     * @author Faramita
     */
    public function delMessageNotice($user_id,$message_id,$category = ''){
        if(!empty($category) || $category == '0'){
            $del_where['category'] = $category;
        }
        if (!empty($user_id) && !empty($message_id)) {
            $del_where['user_id'] = $user_id;
            $del_where['message_id'] = $message_id;

            $del_meg_res = db('user_message')->where($del_where)->delete();
            if ($del_meg_res !== false){
     ...
     * @author Faramita
     */
    public function cleanMessageNotice($user_id,$category = ''){
        if(!empty($category) || $category == '0'){
            $del_where['category'] = $category;
        }
        if (!empty($user_id)) {
            $del_where['user_id'] = $user_id;
            $all_del = db('user_message')->where($del_where)->delete();
            if($all_del !== false){
                return ['status' => 1, 'msg' => '删除成功', 'result' => ''];
            }
        }
