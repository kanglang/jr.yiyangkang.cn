...
class GoodsLogic extends Model
{
        ...
        if($goods_type == 0)
        {
            ...
            foreach($post as $k => $v)
            {
               ...
               foreach ($v as $k2 => $v2)
               {
                   ...
                   $attr_price = isset($post_attr_price[$k2])?$post_attr_price[$k2]:0; 
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $keySpecGoodsPrice = M('SpecGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,price,level0_price,level1_price,level2_price,store_count,bar_code,sku');//规格项
...
//            isset($keySpecGoodsPrice[$item_key]['level0_price']) ? false : $keySpecGoodsPrice[$item_key]['level0_price'] = 0; // 价格默认为0
//            isset($keySpecGoodsPrice[$item_key]['level1_price']) ? false : $keySpecGoodsPrice[$item_key]['level1_price'] = 0; // 价格默认为0
//            isset($keySpecGoodsPrice[$item_key]['level2_price']) ? false : $keySpecGoodsPrice[$item_key]['level2_price'] = 0; // 价格默认为0
			isset($keySpecGoodsPrice[$item_key]['store_count']) ? false : $keySpecGoodsPrice[$item_key]['store_count'] = 0; //库存默认为0
            isset($keySpecGoodsPrice[$item_key]['sku']) ? false : $keySpecGoodsPrice[$item_key]['sku'] = 0;
...
//            $str .="<td><input name=item[$item_key][level0_price] value='{$keySpecGoodsPrice[$item_key]['level0_price']}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
//            $str .="<td><input name=item[$item_key][level1_price] value='{$keySpecGoodsPrice[$item_key]['level1_price']}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
//            $str .="<td><input name=item[$item_key][level2_price] value='{$keySpecGoodsPrice[$item_key]['level2_price']}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
