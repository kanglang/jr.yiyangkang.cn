...
class weixin extends Wxpay_client_pub{

	public $payment_info=array();

	function __construct($pay_code = 'weixin'){
		//
		$paymentPlugin = db('plugin')->where("code='{$pay_code}' and  type = 'payment' ")->find(); // 找到微信支付插件的配置
        ...
        //var_dump($pay_code,$payment_info);exit;
	...
	public function get_payform(&$order_info=[],$openid){
		...
		$unifiedOrder->setParameter("attach","wx_native_app");//	附加数据，在查询API和支付通知中原样返回，可作为自定义参数使用 String(127)
		...
		if (!empty($prepay_id)) {	//返回客户端的参数  
			...
			$info['orderId'] = $order_info['order_sn'];
	 ...
	 *  $param array $order_info 订单信息
	 */
	public function get_wxapp_payform(&$order_info=[],$openid){
		if(!$order_info){
			return false;
		}
		...
		$unifiedOrder->setParameter("attach","wxapp");//附加数据，在查询API和支付通知中原样返回，可作为自定义参数使用 
	 ...
	 *  $param array $order_info 订单信息
	 */
	public function get_wxpulic_payform(&$order_info=[],$openid){
		if(!$order_info){
			return false;
		}
		...
		$unifiedOrder->setParameter("notify_url",API_URL.'paynotify/wxpay_notify');//异步通知地址 
		$unifiedOrder->setParameter("trade_type","JSAPI");//交易类型 JSAPI，NATIVE，APP
		$unifiedOrder->setParameter("attach","wx_pub_app");//	附加数据，在查询API和支付通知中原样返回，可作为自定义参数使用 
