...
use app\shop\model\FlashSale;
use app\shop\model\Goods;
use app\shop\model\GoodsActivity;
use app\shop\model\GroupBuy;
...
//        $data['group'] = (empty($data['group'])) ? '' : implode(',', $data['group']); //前台暂时不用这个功能，先注释
        ...
        if ($prom_id) {
            ...
            if(!empty($data['goods_id']))unset($data['goods_id'],$data['goods']); //2018-07-06 16:51:20
...
//        }
    ...
    public function groupbuyHandle()
    {
        ...
        if (isset($data['act']) && $data['act'] == 'add') {
            unset($data['act']);
        ...
        if (isset($data['act']) && $data['act'] == 'edit') {
            unset($data['act']);
        ...
        if ($r !== false) {
            $this->ajaxReturn(['status' => 1,'msg' =>'操作成功','url' => url('Promotion/group_buy_list')]);
        } else {
    ...
    public function flash_sale_info()
    {
        if (IS_POST) {
            ...
            if (empty($data['id'])) {
                ...
                if(!empty($data['item_id']) && $data['item_id'] > 0){
                    //设置商品一种规格为活动
                ...
                if ($r !== false) {
                    $this->ajaxReturn(['status' => 1, 'msg' => '编辑抢购活动成功', 'url' => url('promotion/flash_sale')]);
                } else {
