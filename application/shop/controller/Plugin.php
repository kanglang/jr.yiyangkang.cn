...
//        $row = $model->where($condition)->save($update);
        ...
        if($condition['type'] == 'payment' && $update['status']){
            ...
            $config = include $file;
            $add['bank_code'] = !empty($config['bank_code'])?serialize($config['bank_code']):'';
            $add['config'] = !empty($config['config'])?serialize($config['config']):'';
            $add['config_value'] = '';
        ...
        foreach($plugin_list as $pt=>$pv){
            ...
            foreach($pv as $t=>$v){
                ...
                if(empty($is_exit)){
                    ...
                    $add['author'] = $v['author'];
                    $add['desc'] = $v['desc'];
                    $add['bank_code'] = empty($v['bank_code']) ? '' : serialize($v['bank_code']);
                    $add['type'] = $pt;
                    $add['scene'] = empty($v['scene']) ? '' : $v['scene'];
    ...
    public function setting(){
        ...
        if(IS_POST){
            ...
            if($row){
                exit($this->success("操作成功",url('Plugin/index')));
            }
            exit($this->error("操作失败",url('Plugin/index')));
        }
        ...
        if(IS_POST){
            ...
            if(I('get.edit') == 1){
                ...
                if($row){
                    ...
                    if(I('post.default') == 1){
                        //默认全国其他地区
                        exit($this->success("添加成功",url('Plugin/shipping_list',array('type'=>'shipping','code'=>$add['shipping_code']))));
                    }
                    M('area_region')->insertAll($add2)&&exit($this->success("添加成功",url('Plugin/shipping_list',array('type'=>'shipping','code'=>$add['shipping_code']))));
                }
    ...
    public function add_shipping()
    {
        ...
        $name = I('name'); // 物流名字
        $desc = I('desc', '');// 插件描述
        $shipping_img = I('shipping_img');
        echo $dir = APP_PATH."shop/plugins/shipping/$code";
        

        if(strstr($code, '(') || strstr($name, '(') || strstr($desc, '('))
        ...
        if(!$shipping_img)$this->error("物流图片图标必传");
