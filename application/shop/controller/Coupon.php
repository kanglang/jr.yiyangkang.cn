...
class Coupon extends Base {
    ...
    public function coupon_info(){
        if (IS_POST) {
            ...
            $goods_id_arr = isset($data['goods_id'])?$data['goods_id']:0;;//商品id
            //2018-07-06 18:16:54
            ...
            if (empty($data['id'])) {
                ...
                if(isset($data['goods_id']))unset($data['goods_id'],$data['goods']);
                //
                ...
                //指定商品分类id
                if($data['use_type'] == 2){
                    $data['cat_id3'] = isset($data['cat_id3'])?$data['cat_id3']:0;
                    Db::name('goods_coupon')->add(['coupon_id'=>$data['id'],'goods_category_id'=>$data['cat_id3']]);
                }
            }
            if ($row !== false) {
                $this->ajaxReturn(['status' => 1, 'msg' => '操作代金券成功', 'url' => url('coupon/index')]);
            } else {
