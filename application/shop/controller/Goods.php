...
class Goods extends Base {
    ...
    public function addEditCategory(){
            ...
            if(I('is_ajax') == 1)
            {
                ...
                if(!$validate->batch()->check(input('post.')))
                {
                    ...
                    if ($type == 2)
                    {
                        $GoodsCategory->allowField(true)->isUpdate(true)->save(); // 写入数据到数据库
                        $GoodsLogic->refresh_cat(I('id'));
                    }
                    else
                    {
                        $GoodsCategory->allowField(true)->save(); // 写入数据到数据库
                    ...
                    $return_arr = array(
                        'status' => 1,
                        'msg'   => '操作成功',
                        'data'  => array('url'=>U('shop/Goods/categoryList')),
                    );
                    $this->ajaxReturn($return_arr);
    ...
    public function delGoodsCategory(){
        ...
        $this->ajaxReturn(['status' => 1,'msg' =>'操作成功','url'=>U('shop/Goods/categoryList')]);
    }
    ...
    public function addEditGoods()
    {
        ...
        if ((I('is_ajax') == 1) && IS_POST) {
            ...
            $return_url = $is_distribut > 0 ? U('shop/Distribut/goods_list') : U('shop/Goods/goodsList');
            $data = input('post.');
            ...
            $data['exchange_integral'] = !empty($data['is_virtual']) && ($data['is_virtual'] == 1) ? 0 : $data['exchange_integral'];
            ...
            $price_ladder = array();
            if (!empty($Goods->ladder_amount) && $Goods->ladder_amount[0] > 0) {
            ...
            if ($type == 2) {
                ...
                //var_dump($Goods);exit;
                $Goods->allowField(true)->save(); // 写入数据到数据库
    ...
    public function addEditGoodsType()
    {
        ...
        if (IS_POST) {
            ...
            $this->success("操作成功!!!", U('shop/Goods/goodsTypeList'));
            exit;
        }
            ...
            if((I('is_ajax') == 1) && IS_POST)//ajax提交验证
            {
                    ...
                    if(!$validate->batch()->check($post_data))
                    {
                        ...
                        $this->ajaxReturn($return_arr);
                             ...
                             if ($type == 2)
                             {
                                 $model->allowField(true)->isUpdate(true)->save(); // 写入数据到数据库
                             }
                             else
                             {
                                 $model->save(); // 写入数据到数据库
                             ...
                             $return_arr = array(
                                 'status' => 1,
                                 'msg'   => '操作成功',
                                 'data'  => array('url'=>U('shop/Goods/goodsAttributeList')),
                             );
                             $this->ajaxReturn($return_arr);
                }
            }
    ...
    public function updateField(){
        ...
        $table = input('table');
        $field_name = $primary[$table];
        $id_value = input('id');
        $field = input('field');
        $value = input('value');
        db($table)->where("$field_name = $id_value")->save(array($field=>$value)); // 根据条件保存修改的数据
    ...
    public function delGoods()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("shop/goods/goodsList")]);
    }
    ...
    public function delGoodsType()
    {
        ...
        $count > 0 && $this->error('该类型下有商品规格不得删除!',U('shop/Goods/goodsTypeList'));
        // 判断 商品属性
        $count = M("GoodsAttribute")->where("type_id = {$id}")->count("1");
        $count > 0 && $this->error('该类型下有商品属性不得删除!',U('shop/Goods/goodsTypeList'));
        // 删除分类
    ...
    public function delGoodsAttribute()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => "操作成功!",'url'=>U('shop/Goods/goodsAttributeList')]);
    }
    ...
    public function delGoodsSpec()
    {
        ...
        $this->ajaxReturn(['status' => 1,'msg' => "操作成功!!!",'url'=>U('shop/Goods/specList')]);
    }
    ...
    public  function addEditBrand(){
            ...
            if(IS_POST)
            {
                ...
                $this->ajaxReturn(['status'=>1,'msg'=>'操作成功','url'=>url('goods/brandList')]);
            }
           $cat_list = M('goods_category')->where("parent_id = 0")->select(); // 已经改成联动菜单
           $this->assign('cat_list',$cat_list);
    ...
    public function delBrand()
    {
        ...
        if($res){
            $this->ajaxReturn(['status' => 1,'msg' => '操作成功','url'=>U("shop/goods/brandList")]);
        }
    ...
    public  function addEditSpec(){
            ...
            if((I('is_ajax') == 1) && IS_POST)//ajax提交验证
            {
                ...
                if ($scene == 'edit') {
                    ...
                    $model->allowField(true)->save(); // 写入数据到数据库
                    $insert_id = $model->getLastInsID();
