...
class Order extends Base {
    ...
    public function ajaxindex(){
        ...
        $condition['order_prom_type'] = array('in',array('0','1','2','3','4','7'));
...
exit("功能正在开发中。。。");
    ...
    public function detail($order_id){
        ...
        if($adminIds && count($adminIds) > 0){
            $admins = M("admin")->where("id in (".implode(",",$adminIds).")")->getField("id , username", true);
        }
        if($has_user){
    ...
    public function edit_order(){
       	...
       	if(IS_POST)
        {
            ...
            if(!empty($order['address2']))unset($order['address2']);
            $o = M('order')->where('order_id='.$order_id)->save($order);

            $l = $orderLogic->orderActionLog($order_id,'修改订单','修改订单');//操作日志
            if($o && $l){
            	$this->success('修改成功',U('shop/Order/editprice',array('order_id'=>$order_id)));
            }else{
            	$this->success('修改失败',U('shop/Order/detail',array('order_id'=>$order_id)));
            }
            exit;
        }
        // 获取省份
    ...
    public function split_order(){
    	...
    	if(IS_POST){
    		...
    		foreach($brr as $goods){
    			...
    			unset($new_order['order_id'],$new_order['address2']);
    			$new_order_id = DB::name('order')->insertGetId($new_order);//插入订单表
    			foreach ($goods as $vv){
    				$vv['order_id'] = $new_order_id;
    				unset($vv['rec_id']);

    				$nid = \think\loader::model('OrderGoods')->allowField(true)->save($vv);//插入订单商品表
    			}
    		}
    		//################################新单处理结束
    ...
    public function editprice($order_id){
        ...
        if(IS_POST){
            ...
            if(!$row){
                $this->success('没有更新数据',U('shop/Order/editprice',array('order_id'=>$order_id)));
            }else{
                $this->success('操作成功',U('shop/Order/detail',array('order_id'=>$order_id)));
            }
            exit;
    ...
    public function delivery_info(){
		...
		$delivery_record = M('delivery_doc')->alias('d')->join('__ADMIN__ a','a.id = d.admin_id')->where('d.order_id='.$order_id)->select();
		if($delivery_record){
    ...
    public function refund_balance(){
		...
		$this->ajaxReturn(['status'=>1,'msg'=>"操作成功",'url'=>U("shop/Order/return_list")]);
    ...
    public function add_return_goods()
   {
    ...
    public function order_action(){
        ...
        if($action && $order_id){
        	 ...
        	 if($res !== false && $a !== false){
                 if ($action == 'remove') {
                     exit(json_encode(array('status' => 1, 'msg' => '操作成功', 'data' => array('url' => U('shop/order/index')))));
                 }
        	 	exit(json_encode(array('status' => 1,'msg' => '操作成功')));
        	 }else{
                 if ($action == 'remove') {
                     exit(json_encode(array('status' => 0, 'msg' => '操作失败', 'data' => array('url' => U('shop/order/index')))));
                 }
    ...
    public function order_log(){
        ...
        //$admin = M('admin')->field('id as admin_id,username as user_name')->find();
        //var_dump($admin);exit;
	    ...
	    if(is_array($orderList)){
	    	...
	    	foreach($orderList as $k=>$val){
	    		...
	    		foreach($orderGoods as $goods){
	    			...
	    			$strGoods .= "商品编号：".$goods['goods_sn']." 商品名称：".$goods['goods_name']." 商品数量：".$goods['goods_num'];
        ...
        if(IS_POST)
        {
           ...
           $order['total_amount']   = $result['result']['total_amount']; // 订单总价
            ...
            if($order_id)
            {
                ...
                $this->success('添加商品成功',U("shop/Order/detail",array('order_id'=>$order_insert_id)));
                exit();
            }
            else{
                $this->error('添加失败');
