...
class Comment extends Base {
    ...
    public function commentHandle(){
        $type = input('type');
        $ids = !empty(input('selected'))?input('selected',''):input('id','');
        if(!in_array($type, array('del', 'show', 'hide')) || empty($ids)){
            $this->error('非法操作！',url('Comment/index'));
        }
        ...
        if($row !== false){
            $this->success('操作完成',url('Comment/index'));
        }else{
            $this->error('操作失败',url('Comment/index'));
        }
    }
    ...
    public function ask_handle()
    {
        ...
        if($row !== false){
            $this->success('操作完成',url('Comment/ask_list'));
        }else{
            $this->error('操作失败',url('Comment/ask_list'));
        }
    }
}