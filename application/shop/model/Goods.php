...
//         M("SpecGoodsPrice")->where('goods_id = '.$goods_id)->delete(); // 删除原有的价格规格对象
        ...
        if ($goods_item) {
            ...
            foreach ($goods_item as $k => $v) {
                ...
                $v['sku'] = trim($v['sku']);
                $data = ['goods_id' => $goods_id, 'key' => $k, 'key_name' => $v['key_name'], 'price' => $v['price'],'level0_price' => $v['level0_price'],'level1_price' => $v['level1_price'],'level2_price' => $v['level2_price'], 'store_count' => $v['store_count'], 'sku' => $v['sku']];
