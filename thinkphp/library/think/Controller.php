...
class Controller
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        if(!defined('MODULE_NAME'))define('MODULE_NAME',$this->request->module());  // 当前模块名称是
        if(!defined('CONTROLLER_NAME'))define('CONTROLLER_NAME',$this->request->controller()); // 当前控制器名称
        if(!defined('ACTION_NAME'))define('ACTION_NAME',$this->request->action()); // 当前操作名称是
        if(!defined('PREFIX'))define('PREFIX',C('database.prefix')); // 数据库表前缀               
        if(!defined('ACTION_NAME'))$this->assign('action',ACTION_NAME);	
        // 控制器初始化
        $this->_initialize();
