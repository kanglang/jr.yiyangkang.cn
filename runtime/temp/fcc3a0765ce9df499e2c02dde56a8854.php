<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:97:"/www/wwwroot/jr.yiyangkang.cn/public/../application/shop/view/sms_template/sms_template_list.html";i:1554712285;s:70:"/www/wwwroot/jr.yiyangkang.cn/application/shop/view/public/header.html";i:1554712283;s:45:"../application/common/view/public/header.html";i:1554712305;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <div class="container-fluid">
      <div class="ibox float-e-margins">
        <div class="ibox-title">
          <h3 class="panel-title"><i class="fa fa-list"></i> 阿里大鱼短信配置模板</h3>
        </div>
        <div class="ibox-content">    
		<div class="navbar navbar-default">                    
               	<div class="form-group">     
	                <button type="button" class="btn btn-primary pull-right"  onclick="location.href='<?php echo url('SmsTemplate/addEditSmsTemplate'); ?>'">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                            ...
                            <?php if(is_array($smsTplList) || $smsTplList instanceof \think\Collection || $smsTplList instanceof \think\Paginator): $i = 0; $__LIST__ = $smsTplList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
                                <tr>
                                    <td class="text-right"><?php echo $list['tpl_id']; ?></td>
                                    <td class="text-left"> <?php echo $send_scene[$list['send_scene']][0]; ?></td>
                                    <td class="text-left"><?php echo $list['sms_sign']; ?></td>
                                    <td class="text-left"><?php echo $list['sms_tpl_code']; ?></td>
                                    <td class="text-left"><?php echo $list['tpl_content']; ?></td>
                                    <td class="text-left"><?php echo date('Y-m-d H:i:s',$list['add_time']); ?></td>
                                    <td class="text-left">
                                        <a href="<?php echo url('SmsTemplate/addEditSmsTemplate', array('tpl_id' => $list['tpl_id'])); ?>" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="编辑"><i class="fa fa-pencil"></i></a>
                                        <a href="javascript:void(0);" onclick="del('<?php echo $list["tpl_id"]; ?>')" id="button-delete6" data-toggle="tooltip" title="" class="btn btn-danger" data-original-title="删除"><i class="fa fa-trash-o"></i></a>
                                    </td>
                                </tr>
...
function del(id)
{
	...
	layer.confirm('确定要删除吗？', {
		...
		$.ajax({										  
			url:"<?php echo url('SmsTemplate/delTemplate'); ?>?id="+id,
			success: function(v){
                layer.closeAll();
				var v =  eval('('+v+')');                                 
                if(v.hasOwnProperty('status') && (v.status == 1)){
                	 location.href='<?php echo url('smsTemplate/index'); ?>';
                }else{                                
