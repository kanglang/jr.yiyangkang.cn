<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:64:"/www/wwwroot/jr.yiyangkang.cn/addons/editorforadmin/content.html";i:1554712245;}*/ ?>
<?php switch($addons_config['editor_type']): case "1": ?>
	...
	<?php break; case "2": ?>
		...
		<?php if($addons_config['editor_wysiwyg'] == '1'): ?>
			<link rel="stylesheet" href="__JS__/plugins/kindeditor/default/default.css" />
			<script charset="utf-8" src="__JS__/plugins/kindeditor/kindeditor-min.js"></script>
			<script charset="utf-8" src="__JS__/plugins/kindeditor/zh_CN.js"></script>
			<script type="text/javascript">
				...
				KindEditor.ready(function(K) {
					editor_<?php echo $addons_data['name']; ?> = K.create('textarea[name="<?php echo $addons_data['name']; ?>"]', {
						...
						uploadJson : '<?php echo addon_url("Editorforadmin://Upload/ke_upimg"); ?>',
		...
		<?php else: ?>
        	...
        	<link rel="stylesheet" href="__JS__/plugins/<?php echo $driver_file; ?>/themes/default/css/ueditor.css" />
			<script type="text/javascript" charset="utf-8" src="__JS__/plugins/<?php echo $driver_file; ?>/ueditor.config.js"></script>
			<script type="text/javascript" charset="utf-8" src="__JS__/plugins/<?php echo $driver_file; ?>/ueditor.all.js"></script>
			<script type="text/javascript" charset="utf-8" src="__JS__/plugins/<?php echo $driver_file; ?>/lang/zh-cn/zh-cn.js"></script>
         	
			<script type="text/javascript">
				$('textarea[name="<?php echo $addons_data['name']; ?>"]').attr('id', 'editor_id_<?php echo $addons_data['name']; ?>');
				window.UEDITOR_HOME_URL = "__JS__/plugins/<?php echo $driver_file; ?>/";
				...
				window.UEDITOR_CONFIG.imageUrl = '<?php echo addon_url("Editorforadmin://Upload/ue_upimg"); ?>';
				window.UEDITOR_CONFIG.imagePath = '';
				window.UEDITOR_CONFIG.imageFieldName = 'file';
				//在这里扫描图片
				window.UEDITOR_CONFIG.imageManagerUrl='<?php echo addon_url("Editorforadmin://Upload/ue_mgimg"); ?>';//图片在线管理的处理地址
                ...
                'link', 'unlink',  'insertimage', 'insertvideo','wordimage', 'emotion', 'attachment', 'map','preview']
						],
				...
				<?php if(!empty($addons_data['btnClassName'])): ?>
						 ...
						 <?php else: ?>
							...
							var srcIds = "";
							 ...
							 $(".<?php echo $addons_data['name']; ?>_preview").append(imghtml);
							  ...
							  //多张图片存储一逗号分隔的id串
							  $("#editor_id_<?php echo $addons_data['name']; ?>").text(oldIds+","+srcIds);
						 <?php endif; ?>
					  })
					  //增加按钮className
					  <?php if(empty($addons_data['btnClassName'])): ?>
...
<!--             <script type="text/javascript" charset="utf-8" src="__JS__/plugins/<?php echo $driver_file; ?>/h5/main.js"></script> -->
		<?php endif; break; case "3": ?>
		
		<script type="text/javascript" src="__JS__/plugins/jquery-migrate-1.2.1.min.js"></script>
		<script charset="utf-8" src="__JS__/plugins/xheditor/xheditor-1.2.1.min.js"></script>
		<script charset="utf-8" src="__JS__/plugins/xheditor/xheditor_lang/zh-cn.js"></script>
		<script type="text/javascript" src="__JS__/plugins/xheditor/xheditor_plugins/ubb.js"></script>
		<script type="text/javascript">
	...
	<?php break; case "4": ?>
		<link rel="stylesheet" href="__JS__/plugins/thinkeditor/skin/default/style.css" />
		<script type="text/javascript" src="__JS__/plugins/jquery-migrate-1.2.1.min.js"></script>
		<script type="text/javascript" src="__JS__/plugins/thinkeditor/jquery.thinkeditor.js"></script>
		<script type="text/javascript">
			$(function(){
				$('textarea[name="<?php echo $addons_data['name']; ?>"]').attr('id', 'editor_id_<?php echo $addons_data['name']; ?>');
				var options = {
					...
					"uploader": "<?php echo addon_url('Editor://Upload/upload'); ?>"
			    };
			    $('#editor_id_<?php echo $addons_data['name']; ?>').thinkeditor(options);
			})
		</script>
		<input type="hid<den" name="parse" value="2">
	<?php break; endswitch; ?>