<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:86:"/www/wwwroot/jr.yiyangkang.cn/public/../application/shop/view/goods/ajaxGoodsList.html";i:1554712288;}*/ ?>
        ...
        <table class="table table-bordered table-hover">
            ...
            <?php if(is_array($goodsList) || $goodsList instanceof \think\Collection || $goodsList instanceof \think\Paginator): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
                <tr>
                    ...
                    <td class="text-center">                        
                        <img width="20" height="20" src="__SHOP_URL__/images/<?php if($list['is_on_sale'] ==1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal('goods','goods_id','<?php echo $list['goods_id']; ?>','is_on_sale',this)"/>
                    </td>
                    <td class="text-center">
                        <img width="20" height="20" src="__SHOP_URL__/images/<?php if($list['is_recommend'] ==1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal('goods','goods_id','<?php echo $list['goods_id']; ?>','is_recommend',this)"/>
                    </td>
                    <!-- <td class="text-center">
                        <img width="20" height="20" src="__SHOP_URL__/images/<?php if($list['is_new'] ==1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal('goods','goods_id','<?php echo $list['goods_id']; ?>','is_new',this)"/>
                    </td>
                    <td class="text-center">
                        <img width="20" height="20" src="__SHOP_URL__/images/<?php if($list['is_hot'] ==1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal('goods','goods_id','<?php echo $list['goods_id']; ?>','is_hot',this)"/>
                    </td>  -->                                       
                    <td class="text-center">                         
                    ...
                    <td class="text-right">
                        <a href="<?php echo U('shop/Goods/addEditGoods',array('id'=>$list['goods_id'])); ?>" class="btn btn-primary" title="编辑"><i class="fa fa-pencil"></i></a>
...
<script>
    ...
    function ClearGoodsHtml(goods_id)
    {
    	$.ajax({
				type:'GET',
				url:"<?php echo U('shop/System/ClearGoodsHtml'); ?>",
    ...
    function ClearGoodsThumb(goods_id)
    {
    	$.ajax({
				type:'GET',
				url:"<?php echo U('shop/System/ClearGoodsThumb'); ?>",
				data:{goods_id:goods_id},
				dataType:'json',
				success:function(data){
					layer.alert(data.msg, {icon: 2});								 
